	...
	public applicationBean getSpecificApplication(String id) {
		...
		if (flag) {
			...
			resultBean.setId(element.getElementsByTagName("id").item(0)
					.getTextContent());
			resultBean.setLink(element.getElementsByTagName("link").item(0)
					.getTextContent());
			resultBean.setCoverLetter(element
					.getElementsByTagName("coverLetter").item(0)
					.getTextContent());
			resultBean.setStatus(element.getElementsByTagName("status").item(0)
					.getTextContent());
			resultBean.setFullname(element.getElementsByTagName("fullname")
					.item(0).getTextContent());
			resultBean.setStatus(element.getElementsByTagName("status").item(0)
					.getTextContent());
			resultBean.setAddress(element.getElementsByTagName("address")
					.item(0).getTextContent());
			resultBean.setLicense(element.getElementsByTagName("license")
					.item(0).getTextContent());
			return resultBean;
		} else {
			return null;
