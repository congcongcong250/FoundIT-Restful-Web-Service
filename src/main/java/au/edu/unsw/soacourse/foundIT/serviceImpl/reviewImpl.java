...
public class reviewImpl implements reviewService {
	...
	public reviewBean getSpecificReview(String id) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			if (tempList.get(i).getReviewId().equals(id)
					&& tempList.get(i).getReviewId() != null)
				flag = true;
		}
		if (flag) {
			Element element = (Element) revDAO.selectSingleNode(id);
			resultBean.setReviewId(element.getElementsByTagName("reviewId")
					.item(0).getTextContent());
			resultBean.setAppId(element.getElementsByTagName("appId").item(0)
					.getTextContent());
			resultBean.setuId(element.getElementsByTagName("uId").item(0)
					.getTextContent());
			resultBean.setComments(element.getElementsByTagName("comments")
					.item(0).getTextContent());
			resultBean.setDecision(element.getElementsByTagName("decision")
					.item(0).getTextContent());
			return resultBean;
		} else {
			return null;
	...
	public List<reviewBean> getAppReviews(String appId) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			reviewBean resultBean = new reviewBean();
			if (tempList.get(i).getAppId().equals(appId)) {
				resultBean = tempList.get(i);
	...
	public List<reviewBean> getReviewerReviews(String uId) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			if (tempList.get(i).getuId().equals(uId)
					&& tempList.get(i).getuId() != null) {
				// flag=true;
				System.out.println(tempList.get(i).getReviewId());
				Element element = (Element) revDAO.selectSingleNode(tempList
						.get(i).getReviewId());
				resultBean.setReviewId(element.getElementsByTagName("reviewId")
						.item(0).getTextContent());
				resultBean.setAppId(element.getElementsByTagName("appId")
						.item(0).getTextContent());
				resultBean.setuId(element.getElementsByTagName("uId").item(0)
						.getTextContent());
				resultBean.setComments(element.getElementsByTagName("comments")
