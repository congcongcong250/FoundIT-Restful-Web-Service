...
public class jobPostingServiceProvider {
	...
	public void deletePosting(@PathParam("Id") String id) {
		...
		if (!flag)
			throw new RuntimeException("DELETE: posting with " + id
					+ " not found");
	...
	@Path("/searchJob")
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public List<jobPostingBean> searchJobs(
		...
		if (keyword == null)
			keyword = "";
		List<jobPostingBean> result = jobPostingImpl.searchJobByKeyWord(
				salaryRate, positionType, location, status, keyword);
		return result;

	}

	@GET
	@Path("/searchCompanyPost")
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public List<jobPostingBean> searchJobsbyCompany(@QueryParam("company") String company) {
		// System.out.println(location+"22222222ddd");

		List<jobPostingBean> result = jobPostingImpl
				.searchJobByCompany(company);
		return result;
