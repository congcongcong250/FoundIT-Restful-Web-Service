...
public class reviewServiceProvider {
	...
	public List<reviewBean> getAppReviews(@PathParam("appId") String id) {
		...
		if (b == null) {
			throw new RuntimeException("GET: application with " + id
					+ " has no review");
		}
		return b;
	}
	...
	public List<reviewBean> getReviewerReviews(@PathParam("uId") String uId) {
		...
		if (b == null) {
			throw new RuntimeException("GET: reviewer with " + uId
					+ " has no review");
		}
		return b;
	}
	...
	public void newReview(
		...
		List<reviewBean> list = reviewImpl.getAppReviews(appId);
		if (list.size() == 2) {
			applicationImpl appimpl = new applicationImpl();
			applicationBean app = appimpl.getSpecificApplication(appId);
			reviewBean a = list.get(0);
			reviewBean b = list.get(1);

			if (a.getDecision().equals("pass")
					&& b.getDecision().equals("pass")) {
				app.setStatus("shortlisted");
			} else {
				app.setStatus("fail");
			}
			appimpl.updateApplication(app);
		}
